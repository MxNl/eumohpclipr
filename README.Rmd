...
```
In this case, `eumohp_clipped_countries` contains `r eumohp_clipped_countries |> length()` stars proxy objects because we requested 4 hydrologic orders (`hydrologic_order = 1:4`) and 2 measures (`abbreviation_measure = c("dsd", "lp")`). 2 * 4 = `r eumohp_clipped_countries |> length()`.
